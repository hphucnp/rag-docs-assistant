    ...
    async def generate(
        ...
        *,
        ...
        if not self._api_key:
            raise ChatConfigurationError("GROQ_API_KEY is not configured")
        ...
        if response.status_code == 429:
            raise ChatRateLimitError("Groq chat endpoint is rate-limited")
        ...
        if not choices:
            raise ChatServiceError("Groq response has no choices")
